...
class SimpleDataManager(DataManager):  # Full class
  ...
  def get_data_loader(self, data_file, aug):
    ...
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=self.batch_size, shuffle=True, num_workers=8, pin_memory=True)
    return data_loader
...
class SetDataManager(DataManager):  # Few-Shot
  ...
  def get_data_loader(self, data_file, aug):
    ...
    elif 'val' in data_file:  # Single domain
      dataset = SetDataset(data_file, self.batch_size, transform)
      sampler = EpisodicBatchSampler(len(dataset), self.n_way, 300)
    else:
      dataset = SetDataset(data_file, self.batch_size, transform)
      sampler = EpisodicBatchSampler(len(dataset), self.n_way, self.n_eposide)

    data_loader = torch.utils.data.DataLoader(dataset, batch_sampler=sampler, num_workers=8)
    return data_loader
